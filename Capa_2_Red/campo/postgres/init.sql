...
-- -----------------------------------------------------------------
-- IDEMPOTENTE: este script se puede reejecutar sin romper el despliegue
-- (p. ej. al reprovisionar el plugin PostgreSQL en Railway).
...
CREATE TABLE IF NOT EXISTS test_acl (
    ...
    rw INT NOT NULL -- 1: Read/SUB, 2: Write/PUB, 3: Read+Write
);

-- -----------------------------------------------------------------
-- 2. USUARIOS Y ACL (mismo contrato que la Capa 1 / Capa 3 / Capa 4)
-- -----------------------------------------------------------------
-- Semántica del backend postgres de Mosquitto Go Auth:
--   $2 = 1  -> intento de LECTURA  (SUB)
--   $2 = 2  -> intento de ESCRITURA (PUB)
--   rw = 3  -> habilita ambos
-- El firmware ESP32 (Capa 1) se SUSCRIBE a los dos canales de actuador
-- (alerta y comando); por tanto la ACL del nodo debe otorgar LECTURA
-- (rw = 1) sobre AMBOS, o el lazo inverso Capa 4 -> Capa 1 nunca cierra.
...
INSERT INTO test_acl (username, topic, rw) SELECT
    'nodo_chancay_01', 'chancay/cuenca/tiempo_real/nodo_chancay_01', 2
WHERE NOT EXISTS (
    SELECT 1 FROM test_acl WHERE username = 'nodo_chancay_01'
      AND topic = 'chancay/cuenca/tiempo_real/nodo_chancay_01' AND rw = 2
);
-- LECTURA del canal de alertas MAPE-K (rw = 1)
INSERT INTO test_acl (username, topic, rw) SELECT
    'nodo_chancay_01', 'chancay/actuadores/alerta/nodo_chancay_01', 1
WHERE NOT EXISTS (
    SELECT 1 FROM test_acl WHERE username = 'nodo_chancay_01'
      AND topic = 'chancay/actuadores/alerta/nodo_chancay_01' AND rw = 1
);
-- LECTURA del canal de telecontrol inverso (Capa 4 -> Capa 1). Sin esta fila
-- el ESP32 no recibe los comandos del operador y la sirena nunca actúa.
INSERT INTO test_acl (username, topic, rw) SELECT
    'nodo_chancay_01', 'chancay/actuadores/comando/nodo_chancay_01', 1
WHERE NOT EXISTS (
    SELECT 1 FROM test_acl WHERE username = 'nodo_chancay_01'
      AND topic = 'chancay/actuadores/comando/nodo_chancay_01' AND rw = 1
);
...
WHERE NOT EXISTS (
    SELECT 1 FROM test_acl WHERE username = 'simulador_python'
      AND topic = 'chancay/cuenca/historico' AND rw = 2
);
...
ON CONFLICT (username) DO UPDATE SET password_hash = EXCLUDED.password_hash;
-- Lectura global de la cuenca (telemetría tiempo real + histórico)
INSERT INTO test_acl (username, topic, rw) SELECT 'backend_central', 'chancay/cuenca/#', 1
WHERE NOT EXISTS (SELECT 1 FROM test_acl WHERE username = 'backend_central' AND topic = 'chancay/cuenca/#' AND rw = 1);
-- Escritura de órdenes hacia los actuadores (lazo MAPE-K) y telecontrol
INSERT INTO test_acl (username, topic, rw) SELECT 'backend_central', 'chancay/actuadores/alerta/#', 2
WHERE NOT EXISTS (SELECT 1 FROM test_acl WHERE username = 'backend_central' AND topic = 'chancay/actuadores/alerta/#' AND rw = 2);
INSERT INTO test_acl (username, topic, rw) SELECT 'backend_central', 'chancay/actuadores/comando/#', 2
WHERE NOT EXISTS (SELECT 1 FROM test_acl WHERE username = 'backend_central' AND topic = 'chancay/actuadores/comando/#' AND rw = 2);
...
ON CONFLICT (username) DO UPDATE SET password_hash = EXCLUDED.password_hash;
INSERT INTO test_acl (username, topic, rw) SELECT 'dashboard_nodered', 'chancay/cuenca/#', 1
WHERE NOT EXISTS (SELECT 1 FROM test_acl WHERE username = 'dashboard_nodered' AND topic = 'chancay/cuenca/#' AND rw = 1);
INSERT INTO test_acl (username, topic, rw) SELECT 'dashboard_nodered', 'chancay/actuadores/alerta/#', 1
WHERE NOT EXISTS (SELECT 1 FROM test_acl WHERE username = 'dashboard_nodered' AND topic = 'chancay/actuadores/alerta/#' AND rw = 1);
-- Escritura de comandos inversos del Panel de Telecontrol (Capa 4 -> Capa 1)
INSERT INTO test_acl (username, topic, rw) SELECT 'dashboard_nodered', 'chancay/actuadores/comando/#', 2
WHERE NOT EXISTS (SELECT 1 FROM test_acl WHERE username = 'dashboard_nodered' AND topic = 'chancay/actuadores/comando/#' AND rw = 2);
